...
static void xemacps_descriptor_free2(struct net_local *lp)
{
	int size;
...
static int xemacps_descriptor_init2(struct net_local *lp)
{
	int size;
	struct sk_buff *new_skb;
	u32 new_skb_baddr;
...
//	if( lp->phy_dev )
//		phy_stop(lp->phy_dev);
	xemacps_descriptor_free2(lp);

		rc = xemacps_descriptor_init2(lp);
//	if( rc )
//	{
//		printk(KERN_ALERT "DMA Error\n");
//		return;
//	}
...
//	devm_free_irq(device, ndev->irq, ndev);
//	rc=devm_request_irq(device, ndev->irq, &xemacps_interrupt, 0,
//		ndev->name, ndev);
